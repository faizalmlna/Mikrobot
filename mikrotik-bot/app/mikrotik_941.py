...
def interconnect_drc_status(hostname):
    hostname_1 = hostname["941_drc"]
    try:
        combined_output = ""
        ...
        combined_output = f"{create_banner(hostname_1)}\nBGP INTERCONNECT STATUS :\n{output1} \nINTERCONNECT DRC a STATUS:\n{output2}\n \nBGP INTERCONNECT b STATUS : \n {output3}\nINTERCONNECT b STATUS : \n {output4}"
...
def Database_drc_status(hostname):
    hostname_1 = hostname["941_drc"]
    try:
        combined_output = ""
        output2 = execute_ssh_command(hostname_1, 'ping x.x.x.x count=10')
